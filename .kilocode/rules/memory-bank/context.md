...
- `/search` can open an Impact-based strategy recommendation table that compares every enabled matched market, ranks the top five buy-ask/sell-bid opportunities inside the current gross-spread filter, and reports fee-adjusted simple annualized returns; strategy mode adds per-market participation checkboxes plus select-all/select-none controls that re-rank immediately without refetching
...
- [x] **Impact strategy recommendations (2026-08-22)**: A new `寻找策略` control to the left of Top/Impact opens a responsive recommendation table between the search controls and results. It enumerates Impact ask→bid directions across every enabled distinct matched market regardless of asset or quote currency and defaults to the top five opportunities in the inclusive 0.3%–1.5% gross range. In strategy mode the result table adds checked-by-default per-market participation checkboxes plus `全选`/`全不选`; exclusions immediately re-rank the current opportunities without hiding rows, refetching books, or bubbling into chart selection, and invalidating a displayed strategy leg clears its ratio override. Shared Impact notional defaults to $3000 and remains manual-refresh applied because it refetches books; gross-range percentages, total-fee percentage, Spot-only-buy, convergence days, and participation immediately re-filter/recalculate the current Impact results without network requests. Convergence defaults to 3 days and offers 3/7/14/30/90/180-day presets plus custom input. The final column shows fee-adjusted simple annualized return using `(gross − total fee) × 365 / days`. Self-pairs and invalid-depth combinations still fail closed. Verified with 411 tests, typecheck, scoped ESLint, and diff checks.
...
| 2026-08-22 | Added `/search` Impact strategy recommendations: a `寻找策略` button opens a responsive top-five table; defaults are $3000 Impact, 0.3%–1.5% gross range, and 3-day convergence, with 3/7/14/30/90/180-day presets plus custom input. Only Impact requires refresh; gross range, fee, Spot-only-buy, convergence, and checked market participation immediately update current recommendations. Strategy mode adds checked-by-default row controls with select-all/select-none; all enabled results can pair regardless of asset/quote. Final validation passed with 411 tests, typecheck, scoped ESLint, and diff checks. |
