...
- [x] **Combination volatility parity and custom weights (2026-08-31)**: All active `/search` Spread/Ratio combination charts retain aligned raw leg prices and expose shared 波动率平价 / 自定义配比 controls. Inverse-volatility weights use the displayed preset plus visible ECharts dataZoom timestamps, log-return sample volatility, 365-day annualization, and stable zoom preservation; custom positive finite weights apply to OHLC only after confirmation while turnover/funding metadata remains unchanged. Focused Bun tests, typecheck, and scoped lint pass.
