...
4. **Unified direct-first / strict-proxy fallback** — every exchange fetches from the browser first (user IP bypasses cloud egress blocks) via the narrow response-level `runDirectFirst` runner (`src/lib/utils/direct-first.ts`), which owns only status policy, caller-abort preservation, and real per-leg client timeouts — never URL building, envelopes, schedulers, or caches. Fallback fires at most once and only for network/CORS/client-timeout, upstream HTTP 403/451, or final 5xx; HTTP 429 stays on the direct leg with bounded per-exchange backoff (Retry-After honored, cancellable); ordinary 4xx, business-envelope errors, malformed success payloads, and caller aborts never fall back. Same-origin routes (`/api/hyperliquid`, `/api/gate/futures/usdt/*`, `/api/{binance,okx,lighter,bitget,bybit}`, `/api/spot/[exchange]`) pin upstream hosts plus endpoint/parameter allowlists, validate before any upstream I/O, and forward through `proxyFetch`, which may use `PROXY_URL`/`HTTP(S)_PROXY` via `undici.ProxyAgent` on Node (Edge degrades to direct) and fails loudly instead of silently bypassing a configured proxy. Direct and proxied success payloads share one adapter parse contract (Bitget full `{code,msg,data}` envelope; Bybit full `{retCode,result}` envelope; Hyperliquid/Gate bare JSON with shared enrichment).
