...
interface LighterFundingCandlesChartProps {
  symbol: string;
...
// 格式化资金费率
// 注意：0xArchive API 返回的费率已经是百分比格式（如 -0.0004 表示 -0.04%）
function formatFundingRate(rate: number): string {
  return `${rate.toFixed(4)}%`;
}

// 计算年化费率
// 注意：0xArchive API 返回的费率已经是百分比格式
function toAnnualizedRate(rate: number, fundingIntervalSeconds: number = 3600): number {
  const settlementsPerDay = (24 * 3600) / fundingIntervalSeconds;
  return rate * settlementsPerDay * 365;  // 不再乘以 100
}

// 格式化年化费率
function formatAnnualizedRate(rate: number, fundingIntervalSeconds: number = 3600): string {
...
export default function LighterFundingCandlesChart({
  symbol,
  interval,
  candles,
  intervalFundingRates,
  fundingIntervalSeconds = 3600,
}: LighterFundingCandlesChartProps) {
