...
import {
  adf0, analyzePair, ar1HalfLife, btcResidualBeta, estimatePairModel, normalizeAlignedPairCloses,
...
describe("pair statistics", () => {
  ...
  test("custom ratio uses second / first and rolling z warms up and resets at gaps", () => {
    const input = pair(25, (i) => ({ first: Math.exp(3 + 1.5 * i / 10), second: Math.exp(i / 10) }));
    const model = estimatePairModel(input, { mode: "custom", first: 2, second: 3 });
  ...
  test("calculates AR(1) half-life, rolling beta stability, and exact BTC beta", () => {
    ...
    const analysis = analyzePair(series, { mode: "custom", first: 1, second: 1 }, {
    ...
    const arModel = estimatePairModel(arSeries, { mode: "custom", first: 1, second: 1 });
