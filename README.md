...
- **历史数据分析**：在 `/search` 查看资金费率历史及统计指标，按实际结算数据计算累计费率与年化值
...
`/search` 的历史资金费率统计使用精确的半开时间窗口 `[start, end)`：先累加窗口内实际结算费率，再按窗口或 K 线的实际持续时间年化，不使用 bucket 算术平均值乘当前结算周期。不同结算周期（如 8 小时与 1 小时）、错峰结算、空数据桶和真实 0 费率均分别处理；缺失、空白或非法数据不会伪装成 0。该修正同时覆盖搜索结果的 2/7/30 天统计、单市场与双腿组合分析、资金费率图表及套利策略资金费率字段。独立的 `/funding` 页面保持原有行为。
...
- 加强各交易所搜索数据边界：缺失、空白、非有限费率不再伪装成 0；Gate 历史严格验证 cutoff 覆盖；Lighter 使用正确的秒级查询与费率单位；Gate/OKX 预测费率正确传递至套利策略
- 修正“区间累计资金费率”面板的 0 值、无样本和精确选区覆盖提示显示
- 新增资金费率统计、历史分页、覆盖边界、无效值与预测费率回归测试；`/funding` 页面保持原有行为不变
- 验证通过：766 项 Bun 测试、TypeScript 类型检查、ESLint、Next.js 生产构建
...
- 年化资金费率差均值按 `wA×腿1费率 − wB×腿2费率` 加权重算（Spot 腿按 0，混合对按 `perpLeg` 符号区分），保留原有真实样本与区间口径；平均成交额计算与样本数保持不变
- 切换交易对/区间时自动重置为 `1:1` 并拒绝旧图表的延迟回调，缩放时保持视口
- 验证通过：471 项测试、TypeScript 类型检查、ESLint、Next.js 生产构建
